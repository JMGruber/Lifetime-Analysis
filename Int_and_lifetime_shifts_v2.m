...
tic;
clear all; close all;
allfiles = 1:10; % A:B or matrix [] of individual spec files. Use positive integers only!
skipfiles = [0 6 8 10 11 13 15 17 19 22 26 44 45 51 55 56 63 72 77 79 81 82 92 100 103 105 113 116 119 120 121 128 130 135 138 140 142 143 144 151 152 153 159 160 173 174 181 182 183 189 192 194 195 198 199 200]; % ditto
pathname = 'J:\LHC II\20140117 LHCII frozen pH 7 GOC 0_1x GOC 5C 50 mM Hepes 200 nW new apd\half goc double cat\';
...
Nf = 2.5; % noise multiplication factor (1.5 - 2 for good S/N)

thrliveI = 1000;  % threshold for final intensity level (in cps, background included, intbin excluded)
thrlivet = 3;  % minimal survival time (in seconds)
thr2c = 4000;  % intensity threshold when definitely >1 complex (in cps, background included, intbin excluded)
intbin = 1;   % number of intensity values to be binned together (input)---- for new version the definition is slightly different
timeres = 0.03; % time resolution (excl intbin)
fixbg = true; % usually more accurate
guessbg = 50; % estimated background; used to test for SM (in cps). If fixbg=false, set rather too large than to small
...
for tr=allfiles
    ...
    %    trace=dlmread(strcat(readdir,'\trace (',int2str(i),')'));
        [trace,delaytimes] = read_pt3_v4(0.03,readfile,[filename2 int2str(tr) '.pt3']);
        ...
        temptimeres=0.01;    %*****************not timeres but switch to real #counts
...
AllIntLevelHist=[allint AllIntLevelHist];
...
Histfiltered2=[allint(Histindex) Histfiltered2];

%Histfiltered2=[allint Histfiltered2];
allwelltimesselection=alldwelltime(max(AllIntLevelHist(:,2:end).')>15,:);

DecayX=[0 DecayX];
Histfiltered=[DecayX;Histfiltered2];
 [Histfiltered,Isort]=sortrows(Histfiltered);
...
%            trace=dlmread(strcat(readfile,[' (' int2str(allusedtraces(tr)) ')']));
    ...
    xlabel('Dwell time (s)'); ylabel(['Intensity (c/',int2str(temptimeres*1000*intbin),' ms)']);
    title(figtitle);
