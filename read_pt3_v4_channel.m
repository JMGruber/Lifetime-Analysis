%function [trace,delaytimes] = read_pt3_v4_channel(timeres,pathname,filename1,rChannel)
function [trace,delaytimes] = read_pt3_v4_channel(timeres,pathname,filename1,rChannel)

% rChannel: 1 -> Old APD, 2 -> New APD
...
for i=1:Records
	...
	switch chan;
		...
		case 2, cnt_2=cnt_2+1; dtime = bitand(bitshift(T3Record,-16),4095); delaytimes2(cnt_2)=dtime.*Resolution;  % regular count at Ch1, Rt_Ch2 when the router is enabled
...
elseif rChannel == 2
	delaytimes=delaytimes2(1:cnt_2);
end
