...
tic;
clear all; close all;
allfiles = 9:10; % A:B or matrix [] of individual spec files. Use positive integers only!
skipfiles = []; % ditto
pathname = 'D:\Data 2013\Anjali\Second week\friday\pH 7 RT glas sandwhich PLL\';
...
Nf = 2; % noise multiplication factor (1.5 - 2 for good S/N)
...
guessbg = 1500; % estimated background; used to test for SM (in cps). If fixbg=false, set rather too large than to small
...
for tr=allfiles
    ...
    %    trace=dlmread(strcat(readdir,'\trace (',int2str(i),')'));
        [trace,delaytimes] = read_pt3_v4(timeres,readfile,[filename2 int2str(tr) '.pt3']);
