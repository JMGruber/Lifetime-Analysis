...
maxsize = 120/0.01;  % max size of any of the data sets
if ~exist('previousDir.mat')
	chosenDir = uigetdir();
	save('previousDir.mat','chosenDir');
else
	load('previousDir.mat');
	chosenDir = uigetdir(chosenDir);
end
getsettings = settings(chosenDir);
uiwait(getsettings);
setVals = guihandles(getsettings);
hdata = guidata(getsettings);
testDir = get(hdata.txtCurDir,'String');
chosenDir = testDir;
save('previousDir.mat','chosenDir');
close(getsettings)
delete(getsettings)
drawnow
load([testDir '\settings.mat']);
figtitle = '';

% allfiles = 1:266; % A:B or matrix [] of individual spec files. Use positive integers only!
% skipfiles = [60 217 218 238]; % ditto
% pathname = 'C:\CD\Data\Redshift\150nW 70_30 bs pH8\28-04-2014 pH 5\';
% readdir=pathname;
% %[filename1, pathname]=uigetfile('*.pt3', 'T3 Mode data:', 0, 0);
% writedir = [pathname 'results'];
% %outfile = [pathname filename(1:end-4) '.out'];
% filename2 = 'trace';
% 
% routerChannel = 1; % 1 for old APD, 2 for new MPD
% 
...
% 
% ki = [3.25 1.96 1.37 1]; % sigma-deviation for 1..4 consecutive trace points, resp.  Paper: ki=[3.25 1.96 1.37 1] But 10-ms data noisier than "normal" shot noise, prob due to triplets as 1 source
% Nf = 1.8; % noise multiplication factor (1.5 - 2 for good S/N)
% 
...
% 
% figtitle = '';
% highQ = 0; % For high quality trace images
% removePR = 0; Remove previous results
...
UseHistBg=0;
lengthBG=30; %needs to be 3751 long
PlotLifetime=0;
HistThr=1000;
DwelltimeThr=1;
...
if removePR
	dirList = dir(fullfile(writedir));
	for i = 1:length(dirList)
		if ~dirList(i).isdir
			delete([writedir '\' dirList(i).name]);
		end
	end
end
...
timetemp = toc;
if UseHistBg==1
BGHist=dlmread(strcat(readfile,'\bgtrace5_30s.txt'));
end
if Parallel
	...
	for tr=allfiles
		try
			...
			if goodfile
				...
				[trace,delaytimes] = read_pt3_v4_channel(timeres,[readfile '\'],[filename2 int2str(tr) '' '_2.pt3'], routerChannel);
				...
				if tlive<thrlivet/trace(1,1)
					...
					if (~photonburst)&&(~dble)
						...
						IntLevelHist=zeros(length(intstart), 3751);
                        DelaytimeShiftIndices=zeros(1,length(intstart)+1);
						for j = 1:length(intstart)
							...
							if sum(trace(intstart(1):intstart(j)+inttimes(j)-1,2))==0
                            DelaytimeShiftIndices(j+1)= DelaytimeShiftIndices(j);
                            else
                            DelaytimeShiftIndices(j+1) = sum(trace(intstart(1):intstart(j)+inttimes(j)-1,2));
                            end
                            IntLevelHist(j,:) = [hist(delaytimes(DelaytimeShiftIndices(j):DelaytimeShiftIndices(j+1)),DecayX)];
						end
						...
						if ~dble
							...
							a{tr-ffile} = P4vars(:,1);
                            if UseHistBg==1
                            IntLevelHist=IntLevelHist-round(BGHist*inttimes/lengthBG).';
                            end
			...
			if tlive<thrlivet/trace(1,1);
				...
				if (~photonburst)&&(~dble)
					...
					if ~dble
						...
						alldwelltime(1:l,i) = inttimes;
						allstarttime(1:l,i) = intstart;
						AllDelaytimeShiftIndices(1:l+1,i) = DelaytimeShiftIndices(1:l+1);
                        if UseHistBg==1
                        IntLevelHist=IntLevelHist-round(BGHist*inttimes/lengthBG).';
                        end
						AllIntLevelHist=[AllIntLevelHist; IntLevelHist];
...
allint2=allint;
[trace_index,trace_number]=find(allint2<(10*thr2c));
...
Histindex=find(sum(AllIntLevelHist(:,2:end).')>=HistThr&alldwelltime.'>DwelltimeThr);
...
[II,JJ]=find(allint2<(10*thr2c));
Histfilteredindices(:,1)=II(alldwelltime.'>DwelltimeThr&sum(AllIntLevelHist(:,2:end).')>=HistThr);
Histfilteredindices(:,2)=JJ(alldwelltime.'>DwelltimeThr&sum(AllIntLevelHist(:,2:end).')>=HistThr);
Histfiltered2=zeros(length(Histfilteredindices(:,1)),3751);
...
alldwelltimeselection=alldwelltime(Histindex);
allstarttimeselection=allstarttime(Histindex);
allintselection=allint(Histindex);
DecayX=[DecayX(1:3291)];
Histfiltered2=[DecayX;Histfiltered2(:,1:3291)];
ParametersOrigin=[alldwelltimeselection allstarttimeselection allintselection Histfilteredindices Histindex'];
...
% 				end
                ...
                for k = testing(tr,1):testing(tr,2)
					x = [allstarttime(k) allstarttime(k)+alldwelltime(k)];
					y = [allint(k)/timeres allint(k)/timeres]+guessbg;
					plot(x,y,'LineWidth',4,'Color','k');
                end
                %set(gca,'YLim',[0 5000],'LineWidth',2);
                xlabel('Time [s]','FontSize',15)
                ylabel('Fluorescence intensity [cps]','FontSize',15)
                set(gca,'box','off')

                
%                 ax1_pos = get(gca,'Position');
%                 ax1_XLim = get(gca,'XLim');
%                 ax2 = axes('Position',ax1_pos,'XAxisLocation','top','YAxisLocation','right','Color','none','XLim',ax1_XLim,'YLim',[0 4],'YColor','r','LineWidth',2);
%                 ylabel('Lifetime [ns]','FontSize',15)
%                 set(gca,'XTick',[])
%                 hold on;
%                
%                 
%                 for l = testing(tr,1):testing(tr,2)
%                     if any(l==Histindex)
% 					x = [allstarttime(l) allstarttime(l)+alldwelltime(l)];
% 					y = [lifetimes(find(Histindex==l)) lifetimes(find(Histindex==l))];
...
%                 end                   
				...
				if ~highQ
					...
					print(fh,fullfile(writedir,['trace' int2str(allusedtraces(tr)) '.png']),'-dpng','-r900');
				end
				saveas(h,fullfile(writedir,['trace' int2str(allusedtraces(tr)) '.pdf']));
				if mod(tr,10)==0 && tr ~= length(allnc)
					close all;
				end
			end
		else
			for tr=1:length(allnc)
                ...
                set(gca,'XLim',[0 120],'LineWidth',2); %'YLim',[0 4000]
                xlabel('Time [s]','FontSize',15)
                ylabel('Fluorescence intensity [cps]','FontSize',15)
                ...
                ax1_pos = get(gca,'Position');
                ax1_XLim = get(gca,'XLim');
                ax2 = axes('Position',ax1_pos,'XAxisLocation','top','YAxisLocation','right','Color','none','XLim',ax1_XLim,'YLim',[0 1.5],'YColor','r','LineWidth',2);  
                ylabel('Lifetime [ns]','FontSize',15)
                hold on;
                ...
                for k = ri:rf
                    if any(k==Histindex)
					x = [allstarttime(k) allstarttime(k)+alldwelltime(k)];
					y = [lifetimes(find(Histindex==k)) lifetimes(find(Histindex==k))];
					plot(x,y,'Parent',ax2,'LineWidth',4,'Color','r');
                    end
                end          
                end
%                   for k = ri:rf
%                     if any(k==Histindex)
% 					x = [allstarttime(k) allstarttime(k)+alldwelltime(k)];
% 					y = [abs(find(Histindex==k)) abs(find(Histindex==k))];
% 					plot(x,y,'Parent',ax2,'LineWidth',4,'Color','b');
%                     end
%                 end      
				...
				if ~highQ
					...
					print(fullfile(writedir,['trace' int2str(allusedtraces(tr)) '.png']),'-dpng','-r600');
				end
				saveas(h,fullfile(writedir,['trace' int2str(allusedtraces(tr)) '.pdf']));
				if mod(tr,10)==0 && tr ~= length(allnc)
					close all;
...
if ~isempty(allusedtraces)
	...
	dlmwrite(strcat(writedir,'\Decay histograms for intensity levels.txt'),Histfiltered2','\t');
    fid = fopen(strcat(writedir,'\Lifetime fit parameters.txt'), 'w');
    fprintf(fid, 'Dwelltime Starttime   Intensity   Level#  Trace#  Histindex \n\n');
