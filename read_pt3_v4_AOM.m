function [trace,delaytimes,RealTimeHist] = read_pt3_v4_AOM(timeres,pathname,filename1,APD,AOM_Value,HistX,XBounds,Shift,subtrace)
...
ofltime = 0;
cnt_1=0; cnt_2=0; cnt_3=0; cnt_4=0; cnt_Ofl=0; cnt_M=0; cnt_Err=0; cnt_AOM=0; % just counters
WRAPAROUND=65536;

%syncperioda = 1E9/CntRate0;   % in nanoseconds
%syncperiodb = 1E9/76E6;
syncperiod=1E9/76E6;
...
case 1, 
    %if (27E9>(ofltime + nsync)*syncperiod+ dtime*Resolution) && (0.1E9<(ofltime + nsync)*syncperiod+ dtime*Resolution)
        %RealTimeHist=[RealTimeHist (ofltime + nsync)*syncperiod+ dtime*Resolution]; 
        if APD==1
        RealTimeHist=[RealTimeHist rem((ofltime + nsync)*syncperiod+Shift+ dtime*Resolution,AOM_Value)];
        Time=[Time (ofltime + nsync)*syncperiod+ dtime*Resolution];
        %RealTimeHist_abs=[RealTimeHist_abs (ofltime + nsync)*syncperiod+ dtime*Resolution];
        
    %end
        if rem((ofltime + nsync)*syncperiod+Shift + dtime*Resolution,AOM_Value)>0
            cnt_1=cnt_1+1; dtime = bitand(bitshift(T3Record,-16),4095); delaytimes1(cnt_1)=dtime.*Resolution; 
        end
        end
...
case 2,  
    ...
    if rem((ofltime + nsync)*syncperiod+Shift + dtime*Resolution,AOM_Value)>0
            ...
            RealTimeHist=[RealTimeHist rem((ofltime + nsync)*syncperiod+Shift + dtime*Resolution,AOM_Value)]; 
            Time=[Time (ofltime + nsync)*syncperiod+ dtime*Resolution];
            cnt_2=cnt_2+1; dtime = bitand(bitshift(T3Record,-16),4095); delaytimes2(cnt_2)=dtime.*Resolution; 
    end
    end
    
    
    % regular count at Ch1, Rt_Ch2 when the router is enabled
...
%RealTimeHist_abs=RealTimeHist_abs';
%HistX_abs=0:50:5000;
figure(13)
RealTimeHist_Smooth=medfilt1(RealTimeHist,1);
RealTimeHistX=1:length(RealTimeHist_Smooth);
scatter(RealTimeHistX,RealTimeHist_Smooth,20,'+');
% set(gca,'YLim',[2500 4000]);
%set(gca,'XLim',[10000 15000]);

%[HistY,XOut]=hist(RealTimeHist(1:length(RealTimeHist)),100);

 %figure(subtrace);
%bar(XOut,HistY);
%xlim([0 max(RealTimeHist)]);
% test2=diff(RealTimeHist_abs);
% figure(2);
% [HistDiff,XOutDiff]=hist(test2,HistX_abs);
% bar(XOutDiff,HistDiff);
% xlim([0 4800]);
%dlmwrite('D:\IntShifts new Tjaart\result AOM extraction\result',test2,' '); 


%RealTimeHist(cnt_1)=rem((ofltime + nsync).*syncperiod+ dtime.*Resolution,33338.3122)
%outfile2 = [pathname filename(1:end-4)];
RealTimeHist(:,2)=RealTimeHist;
RealTimeHist(:,1)=Time;

dlmwrite(strcat(pathname,strcat('RealTimeHist',filename1)),RealTimeHist,' '); 
