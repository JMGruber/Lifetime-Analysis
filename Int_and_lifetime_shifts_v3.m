...
tic;
clear all; close all;
allfiles = 1:305; % A:B or matrix [] of individual spec files. Use positive integers only!
skipfiles = [18 60 87 103 120 138 158 188 192 203 212 247 256 263 296 298 300 301 305]; % ditto
pathname = 'J:\2014\Joshua and Herman\pH dependence PCD PCA\pH 6\';
APD = 0; % "1" is MPD, "0" is SPCM AQRH 16
readdir=pathname;
%[filename1, pathname]=uigetfile('*.pt3', 'T3 Mode data:', 0, 0);
writedir = [pathname 'Lifetime analysis 300'];
...
thrliveI = 4000;  % threshold for final intensity level (in cps, background included, intbin excluded)
...
drawlevels = false;  % draw intensity levels
...
for tr=allfiles
    ...
    %    trace=dlmread(strcat(readdir,'\trace (',int2str(i),')'));
        [trace,delaytimes] = read_pt3_v4(timeres,readfile,[filename2 int2str(tr) '.pt3'],APD);
...
%            trace=dlmread(strcat(readfile,[' (' int2str(allusedtraces(tr)) ')']));
            ...
            figure; h=plot(trace(:,1),trace(:,2)-allusedtracebg(tr),'g');
            hold on;
            for k = ri:rf
