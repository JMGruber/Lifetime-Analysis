...
tic;
clear all; close all;
allfiles = 6:306; % A:B or matrix [] of individual spec files. Use positive integers only!
skipfiles = []; % ditto
pathname = 'J:\2014\Joshua and Herman\pH dependence PCD PCA\pH 5\';
readdir=pathname;
%[filename1, pathname]=uigetfile('*.pt3', 'T3 Mode data:', 0, 0);
writedir = [pathname 'analysis parallel'];
...
if parallel
  ...
  parfor tr=allfiles
	 ...
	 if goodfile
		...
		[trace,delaytimes] = read_pt3_v4(timeres,readfile,[filename2 int2str(tr) '.pt3'],0);
		  ...
		  if (~photonburst)&&(~dble)
			 ...
			 if ~dble
				...
				test3{tr-ffile} = {l,P4vars(:,1),P4vars(:,2),P4vars(:,3),...
				  ...
				  IntLevelHist,length(intlevels) tr-allfiles(1)};
